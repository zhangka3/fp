...
| 14_full_call.sql | full_call.json | tmp_export.case_productivity_2025 | 全量外呼/接通/满频等日粒度生产力（`dt`、`weeknum`、`case_type`、`rank_type` 等；约近 100 个工作日 × 多类型） | ~700+ |
...
- **时间**：`dt` 最近约 100 天滚动窗口；排除红休、`rank_type` 为「其他」等（见 SQL）。
- **分组**：`dt`、`weekofyear(dt)`、`case_type`、`rank_type`；催收类型 `collector_type in ('EM','BPO','AP')`、`is_completed_t0` 等过滤见 SQL。
- **用途**：全量呼叫/满频/接通率等生产力指标落盘；**当前仓库无对应 `screen_*.py`**，供后续看板或扩展图表使用。
...
1. **提交查询**：对本目录下（当前 **14** 个）SQL 按需提交到 SMART。
2. **等待完成**：轮询至 `FINISHED`。
