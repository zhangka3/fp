...
lag(avg_eff_worktime) over(partition by area_type,scope_x order by p_month) as last_avg_eff_worktime,
case 
when p_month<date_format(date_sub(current_date(), 1), 'yyyy-MM') or lag(avg_eff_worktime) over(partition by area_type,scope_x order by p_month) is null then null
else avg_eff_worktime-lag(avg_eff_worktime) over(partition by area_type,scope_x order by p_month) end as diff_avg_eff_worktime
from base
order by 1,2,4,3
;
